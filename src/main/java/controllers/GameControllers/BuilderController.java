...
public class BuilderController {
    ...
    public Output repairImprovement(Player player, BuilderUnit unit) {
        if (unit.getPlayer() != player)
            return Output.UNIT_NOT_YOURS;
        if (!unit.isAWorker())
            return Output.NOT_A_WORKER;
        if (unit.getIsWorking())
            return Output.WORKER_IS_BUSY;
        // can he improve others improvement ?!
        if (!unit.getPosition().getImprovement().getIsBroken())
            return Output.IMPROVEMENT_IS_NOT_BROKEN;
    ...
    public Output makeARoad(Player player, BuilderUnit unit) {
        ...
        if (unit.getIsWorking())
            return Output.WORKER_IS_BUSY;
        if (unit.getPosition().getHasRoad())
            return Output.TILE_HAS_ROAD;
