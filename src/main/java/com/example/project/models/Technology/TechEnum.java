...
public enum TechEnum {
    ...
    private static final HashMap<TechEnum, Image> images = new HashMap<>();

    static {
        try {
            for (TechEnum techEnum : TechEnum.values()) {
                images.put(techEnum, new Image(String.valueOf(
                        new URL(App.class.getResource("/Image/Game/Tech/" + techEnum.getName() + ".png").toString()))));
            }
    ...
    public static HashMap<TechEnum, Image> getImages() {
        return images;
    }
}